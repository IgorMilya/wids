...
const password = (isRequired?: boolean) => {
  const schema = string()
    ...
    .matches(REGEX.ONE_UPPERCASE_LETTER, 'Password must contain at least one capital letter')
    .matches(REGEX.AT_LEAST_ONE_NUMBER, 'Password must contain at least one number')
    .matches(REGEX.AT_LEAST_ONE_SPECIAL_SYMBOL, 'Password must contain at least one special symbol')
